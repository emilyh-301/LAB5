...
public class BlackJackController implements Initializable {
	...
	private PathTransition CreatePathTransition(Point2D fromPoint, Point2D toPoint, ImageView img) {
		...
		path.getElements().add(new MoveTo(fromPoint.getX()+36, fromPoint.getY()+48)); //img.get stuff doesn't work
		path.getElements().add(new CubicCurveTo(toPoint.getX()*2, toPoint.getY() , toPoint.getX() / 3,
				toPoint.getY(), toPoint.getX()+36, toPoint.getY()+48));
		// path.getElements().add(new CubicCurveTo(0, 120, 0, 240, 380, 240));
		PathTransition pathTransition = new PathTransition();
		pathTransition.setDuration(Duration.millis(2250));
		pathTransition.setPath(path);
		pathTransition.setNode(img);//I'M HAGAN
		pathTransition.setOrientation(PathTransition.OrientationType.NONE); //changed
	...
	private RotateTransition CreateRotateTransition(ImageView img) {
		...
		rotateTransition.setCycleCount(8); //changed
